...
async def test_video_slice_workflow():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.screenshot(path="output/qa/playwright/test_01_home.png")

            # ========== 步骤 1: 点击【视频切片】Tab ==========
            print("✅ 步骤 1: 点击【视频切片】Tab...")
            await page.click("text=🎬 视频切片")
            await page.wait_for_timeout(1500)
            await page.screenshot(path="output/qa/playwright/test_02_video_slice_tab.png")

            # ========== 步骤 2: 输入视频路径 ==========
            print(f"✅ 步骤 2: 输入视频路径...")
            text_input = page.locator("input[key='slice_step0_local_path']")
            await text_input.fill(video_path)
            await page.screenshot(path="output/qa/playwright/test_03_video_path_input.png")

            # ========== 步骤 3: 点击"使用本地文件"按钮 ==========
            print("✅ 步骤 3: 点击'使用本地文件'按钮...")
            await page.click("text=使用本地文件")
            await page.wait_for_timeout(1000)
            await page.screenshot(path="output/qa/playwright/test_04_use_local.png")

            # ========== 步骤 4: 点击"下一步：开始处理" ==========
            print("✅ 步骤 4: 点击'下一步：开始处理'...")
            await page.click("text=下一步：开始处理")
            await page.wait_for_timeout(1000)
            await page.screenshot(path="output/qa/playwright/test_05_step1_page.png")

            # ========== 步骤 5: 点击"开始处理" ==========
            print("✅ 步骤 5: 点击'开始处理'...")
            await page.click("text=🚀 开始处理")
            await page.wait_for_timeout(1000)
            await page.screenshot(path="output/qa/playwright/test_06_processing_started.png")
            ...
            while time.time() - start_time < max_wait:
                try:
                    ...
                    if await success_msg.is_visible(timeout=2000):
                        print("✅ 处理完成！")
                        await page.screenshot(path="output/qa/playwright/test_07_processing_done.png")
                ...
                try:
                    ...
                    if await error_msg.is_visible(timeout=2000):
                        print("❌ 处理失败！")
                        await page.screenshot(path="output/qa/playwright/test_07_processing_error.png")
                        processing_done = False
                        break
                except:
                    pass
            ...
            if not processing_done:
                print("❌ 处理超时！")
                await page.screenshot(path="output/qa/playwright/test_07_processing_timeout.png")
                return
            ...
            await page.wait_for_timeout(1000)
            await page.screenshot(path="output/qa/playwright/test_08_preview_page.png")
            ...
            if await segment_buttons.count() > 0:
                ...
                print("✅ 已点击时间轴片段")
                await page.screenshot(path="output/qa/playwright/test_09_timeline_click.png")
            else:
            ...
            if await restart_btn.is_visible():
                ...
                await page.wait_for_timeout(1000)
                await page.screenshot(path="output/qa/playwright/test_10_restart.png")
                print("✅ 已重新开始")

            # ========== 最终截图 ==========
            print("✅ 截取最终页面...")
            await page.screenshot(path="output/qa/playwright/test_final.png")

            print("\n🎉 测试完成！所有截图已保存到 output/qa/playwright/ 目录")

        except Exception as e:
            print(f"\n❌ 测试过程中出现错误: {e}")
            await page.screenshot(path="output/qa/playwright/test_error.png")
            raise
